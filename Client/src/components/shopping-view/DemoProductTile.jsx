...
function DemoProductTile({ product, handleAddtoCart }) {
  ...
  <CardContent className="p-3 text-sm flex flex-col flex-grow cursor-pointer" onClick={() => handleGetProductDetails(product?._id)}>
    <h3 className="text-center text-[13px] font-medium text-[#2D2D2D] leading-snug truncate mb-1">{product?.title}</h3>
  ...
  <CardFooter>
    {product?.totalStock === 0 ? (
      ...
      <Button
        className="bg-[#54E060] text-[#2D2D2D] rounded-1xl px-6 py-2 w-full"
